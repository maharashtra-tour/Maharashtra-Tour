            ...
            <div className='mx-auto max-w-7xl px-4 py-12 my-8 lg:px-10 bg-lightgrey rounded-3xl relative'>
                ...
                <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 my-2 gap-x-16 lg:gap-x-16'>
                    {Aboutdata.map((item, i) => (
                        <div key={i} className='cursor-pointer  hover:shadow-2xl bg-white rounded-3xl mt-8 pt-10 pl-8 pb-4 pr-6 shadow-md group'>
                            <h4 className='text-4xl font-semibold  text-black mb-5'>{item.heading}</h4>
                            <Image src={item.imgSrc} alt={item.imgSrc} width={100} height={100} className="mb-5" />
                            <h4 className='text-lg font-normal text-black group-hover:text-offwhite mb-2'>{item.paragraph}</h4>
                        </div>
                    ))}
                </div>
            </div>
